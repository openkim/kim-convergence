...
def validate_split(n_samples, train_size, test_size, default_test_size=None):
    ...
    if test_size_int and (test_size >= n_samples or test_size <= 0):
        ...
        raise CVGError(msg)
    elif test_size_float and (test_size <= 0 or test_size >= 1):
    ...
    if train_size_int and (train_size >= n_samples or train_size <= 0):
        ...
        msg += 'and smaller than the number of samples={}'.format(n_samples)
        raise CVGError(msg)
    elif train_size_float and (train_size <= 0 or train_size >= 1):
...
def train_test_split(x, *, test_size=None, train_size=None, seed=None, default_test_size=0.1):
    r"""Split x into random train and test indices.

    Args:
        x (array_like): time series data, array-like of shape
    ...
    n_train, n_test = validate_split(n_samples,
                                     test_size,
                                     train_size,
...
def subsample(x, *, si=None, fft=False, mct=5):
    r"""Sample the uncorrelated subsample of the time series data.
    ...
    Returns:
        list: indices list.
            Indices list of an uncorrelated subsample of the time series data.

    """
    if si is None:
        # Compute the statistical inefficiency for the timeseries
        si = statistical_inefficiency(x, fft=fft, mct=mct)
    else:
        if isinstance(si, str):
            ...
            si = si_func(x, fft=fft, mct=mct)
        elif isinstance(si, float) or isinstance(si, int):
            if (si < 1.0):
    ...
    indices = np.where(_ind.astype(int) < n)
